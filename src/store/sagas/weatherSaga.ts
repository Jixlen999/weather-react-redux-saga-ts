...
function* weatherWorker() {
  ...
  const dailyWeather: IDailyWeather[] = yield call(fetchDailyWeather, placeId);
  yield put(setDailyWeather(dailyWeather));

  const hourlyWeather: IHourlyWeather[] = yield call(fetchHourlyWeather, latitude, longitude);
